...
# なぜこれまで通りの開発手法ではダメなのか

### ソフトウェア開発の現実（CHAOS Report Global (2020)の調査）

| 指標                           | 数値       |
| ------------------------------ | ---------- |
| 予定通りに完了するプロジェクト | 約 **30%** |
| 予算超過・納期遅延             | 約 **50%** |
| 中止されるプロジェクト         | 約 **20%** |
...
- テストが後回しになり、バグが **終盤で大量に判明** する
- ドキュメントは書くが、**動くソフトウェア** がなかなか出てこない、その結果発生する **思ってたものと違う** 問題
- 開発側とビジネス側の **コミュニケーション不足**

→ これらの問題への対策として、XP・アジャイルが生まれた
...
- サイト: [Extreme Programming Explained](http://www.extremeprogramming.org/)
...
## 参考:「エクストリーム」の精神 — エクストリームアイロニング
![bg right:35%](fig/pic1.png)
### エクストリームアイロニング（Extreme Ironing）とは？

- 山頂、崖の上、水中、スカイダイビング中など **過酷な場所でアイロンがけ** をするスポーツ
- 1997年イギリス発祥、世界中に愛好家が存在
- 「日常の退屈な家事」を **極限の環境** に持ち込む
- Wikipedia: [Extreme ironing - Wikipedia](https://en.wikipedia.org/wiki/Extreme_ironing)

### XPとの共通点

| エクストリームアイロニング   | エクストリームプログラミング       |
| ---------------------------- | ---------------------------------- |
| アイロンがけ自体は普通の行為 | コードレビュー・テストは普通の行為 |
| それを **極限まで** 追求する | それを **常に・徹底的に** 実践する |
| 周囲からは「なぜそこまで？」 | 周囲からは「なぜそこまで？」       |

**「良いとわかっていること」を極端にやり切る** — それがExtreme の意味
...
Kent Beck、Robert C. Martin ら17名が署名した **ソフトウェア開発の価値宣言**
Kent Beck はXPの前述の通り提唱者でもある

### 4つの価値

| 左側の価値を認めつつも | **右側により大きな価値を置く** |
| ---------------------- | ------------------------------ |
| プロセスやツール       | **個人と対話**                 |
| 包括的なドキュメント   | **動くソフトウェア**           |
| 契約交渉               | **顧客との協調**               |
...
### 「プロセスのアジャイル」と「技術のアジャイル」の乖離

アジャイルは急速に普及したが、多くの現場で **技術プラクティスが軽視** された

| やっていること                     | 欠けていたもの                       |
| ---------------------------------- | ------------------------------------ |
| スプリント・デイリースタンドアップ | TDD・継続的リファクタリング          |
| ユーザーストーリー・見積もり       | ペアプログラミング・コードレビュー   |
| レトロスペクティブ                 | 継続的インテグレーション・自動テスト |

### Martin Fowlerの指摘:「Flaccid Scrum」
...
# ソフトウェアクラフトマンシップ（2009）

### Uncle Bob（Robert C. Martin）による拡張宣言
...
| アジャイル宣言   | **＋ クラフトマンシップ**       |
| ---------------- | ------------------------------- |
| 動くソフトウェア | **＋ よくできたソフトウェア**   |
| 変化への対応     | **＋ 着実に価値を付加すること** |
| 個人との対話     | **＋ 専門家のコミュニティ**     |
| 顧客との協調     | **＋ 生産的なパートナーシップ** |

アジャイルの価値を **否定ではなく補完** し、XPの技術プラクティスを「プロの規律」として位置づけた

---

# クラフトマンシップとXPの実践

### Uncle Bobが主張する開発者の規律

- **TDD** = 医者が手を洗うのと同じ「当たり前のプロの習慣」
- **ボーイスカウトの原則**: コードを見つけた時より良い状態で残す
- **Clean Code**: 読みやすさは機能と同じくらい重要

### XPプラクティスとの対応

| クラフトマンシップの規律 | 対応するXPプラクティス    |
| ------------------------ | ------------------------- |
| テスト駆動開発           | TDD（Red-Green-Refactor） |
| 継続的な改善             | リファクタリング          |
| コードの共同所有         | ペアプログラミング        |
| 小さなリリース           | 継続的インテグレーション  |

**本研修では、演習を通じてこれらの規律を体験します**
...
# t-wadaさんに学ぶTDDの本質
![top-right](fig/pic2.png)
### 和田卓人（t-wada）氏
- Kent Beck『テスト駆動開発』の日本語訳者（2017年）
- 日本におけるTDD普及の第一人者
- TDD Boot Camp の主催・講師として全国で活動

### TDDのゴール

> **動作するきれいなコード**（Clean code that works）— Kent Beck

TDDは単なるテスト手法ではなく、**「動作するきれいなコード」** に到達するための開発手法

### TDDの2つのルール（Kent Beck）
1. **自動化されたテストが失敗したときのみ、新しいコードを書く**
2. **重複を除去する**

この2つのルールから Red-Green-Refactor のサイクルが導かれる
...
> プログラマが感じる **不安** や **不確実性** をテストコードとして表現する
> テストは「仕様の確認」であると同時に「開発者の不安の解消」でもある

### TDDで得られるもの

| 効果                     | 説明                                       |
| ------------------------ | ------------------------------------------ |
| **即座のフィードバック** | コードが正しく動くか数秒でわかる           |
| **変更への自信**         | テストがあるから安心してコードを変更できる |
| **設計の改善**           | テストしやすいコード ＝ 疎結合で良い設計   |
| **生きたドキュメント**   | テストコードが仕様書の役割を果たす         |

### 「質とスピード」はトレードオフではない

- 質を落として速度を上げる → **技術的負債** が蓄積し、やがて速度も落ちる
- TDDで品質を保つ → **長期的にも速度を維持できる**
- 参考: [質とスピード - 和田卓人](https://speakerdeck.com/twada/quality-and-speed)

---

# TDDのよくある誤解（t-wada氏の指摘）

### 誤解と真実

| よくある誤解                     | 真実                                             |
| -------------------------------- | ------------------------------------------------ |
| TDDはテスト手法である            | TDDは **開発手法**（設計とフィードバックの手法） |
| テストを書くと開発が遅くなる     | 短期的コストはあるが **長期的には速くなる**      |
| テストカバレッジ100%を目指すべき | カバレッジは指標に過ぎず **テストの質** が重要   |
| 最初から完璧なテストを書くべき   | テストも **段階的に改善** していけばよい         |
| すべてのコードにTDDを適用すべき  | **不安を感じる部分** から始めればよい            |

### t-wada氏の推薦書籍
- Kent Beck『テスト駆動開発』（オーム社, 2017） — TDDの原典
- Michael Feathers『レガシーコード改善ガイド』 — テストのない既存コードへの対処
- Martin Fowler『リファクタリング』 — TDDの Refactor ステップを深く理解する
...
- Kent Beck "テスト駆動開発"（和田卓人 訳, オーム社, 2017）
- Michael Feathers "レガシーコード改善ガイド"（翔泳社, 2009）

### オンラインリソース
- Agile Alliance: agilealliance.org
- Martin Fowler's Blog: martinfowler.com
- Uncle Bob's Blog: blog.cleancoder.com
- t-wada「質とスピード」: speakerdeck.com/twada/quality-and-speed
- t-wada「TDD Boot Camp」: devtesting.jp/tddbc
